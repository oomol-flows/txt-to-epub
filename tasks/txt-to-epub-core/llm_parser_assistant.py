...
class HybridParser:
    ...
    def __init__(
        ...
        :param llm_api_key: OpenAI API密钥
        :param llm_model: LLM模型
        :param llm_base_url: API基础URL (可选)
        :param config: 解析器配置
        """
        ...
        # 初始化LLM助手
        if llm_api_key or self.config.enable_llm_assistance:
            try:
                ...
                logger.info("LLM助手已启用")
            except ImportError as e:
                logger.warning(f"LLM助手初始化失败: {e}")
                self.llm_assistant = None
        else:
            self.llm_assistant = None
            logger.info("LLM助手未启用")

    def parse(self, content: str, context=None):
        """
        混合解析流程
        ...
        volumes = parse_hierarchical_content(content, self.config, context=context)

        # 如果未启用LLM或解析结果满意,直接返回
        if not self.llm_assistant:
            logger.info("LLM助手未启用,使用规则解析结果")
            return volumes
...
class HybridParser:
    ...
    def parse(self, content: str, skip_toc_removal: bool = False, context=None):
        ...
        # 阶段0: 使用LLM识别并移除目录页
        if self.llm_assistant:
            print(">>> [HybridParser] 阶段0: LLM识别目录页...")
        ...
        if confidence >= threshold:
            print(f">>> [HybridParser] 置信度 >= 阈值, 跳过章节级LLM辅助 (节省成本)")
            logger.info(f"高置信度 ({confidence:.2f} >= {threshold:.2f}), 跳过章节级LLM辅助")
            logger.info(f"解析完成: {len(volumes)} 卷")
            return volumes

        print(f">>> [HybridParser] 置信度 < 阈值, 需要LLM辅助章节识别")
        ...
        # 打印规则解析结果
        print(f"\n>>> [规则解析结果] 识别到 {len(chapters)} 个章节:")
        for i, ch in enumerate(chapters[:10]):  # 只显示前10个
            chapter_obj = ch.get('chapter')
            if chapter_obj:
                print(f"  {i+1}. {chapter_obj.title} (置信度: {ch.get('confidence', 0):.2f})")
            else:
                print(f"  {i+1}. N/A")
        if len(chapters) > 10:
        ...
        if uncertain_regions and self.llm_assistant:
            print(f"\n>>> [HybridParser] 阶段2: LLM辅助处理 {len(uncertain_regions)} 个不确定区域...")
            ...
            # 打印LLM决策
            print(f"\n>>> [LLM决策结果] 处理了 {len(llm_decisions)} 个候选:")
            for i, decision in enumerate(llm_decisions[:10]):
                # 从对应的candidate获取标题
                title = candidates[i].text if i < len(candidates) else 'N/A'
                print(f"  {i+1}. '{title}' - {'✓ 是章节' if decision.is_chapter else '✗ 不是'} (置信度: {decision.confidence:.2f})")
                print(f"      原因: {decision.reason}")
            if len(llm_decisions) > 10:
                print(f"  ... 还有 {len(llm_decisions) - 10} 个决策")
